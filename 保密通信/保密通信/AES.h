#pragma once
#include <stdio.h>
#include <iostream>
#include <stdlib.h>
#include <string.h>
using namespace std;


unsigned char mixColumnMatrix[4][4] = {
    {0x02, 0x03, 0x01, 0x01},
    {0x01, 0x02, 0x03, 0x01},
    {0x01, 0x01, 0x02, 0x03},
    {0x03, 0x01, 0x01, 0x02}
};

unsigned char invmixColumnMatrix[4][4] = {
    {0x0e, 0x0b, 0x0d, 0x09},
    {0x09, 0x0e, 0x0b, 0x0d},
    {0x0d, 0x09, 0x0e, 0x0b},
    {0x0b, 0x0d, 0x09, 0x0e}
};

int sbox[256] = {
    
};
int invsbox[256] = {
    
};

// The round constant word array, Rcon[i], contains the values given by 
// x to the power (i-1) being powers of x (x is denoted as {02}) in the 
// field GF(28).  Note that i starts at 1, not 0).
//轮常量,i从1开始
int Rcon[255] = {
         
};


#define Nb 4// state的字节数4——128bit
int Nr = 0;// 轮数
int Nk = 0;//密钥字节数

unsigned char in[1024], out[1024], state[4][Nb], IV[1024];//输入、输出、中间状态、CBC加密的初始向量
unsigned char RoundKey[240];//扩展子密钥，最大：15轮，256bit——16字节，16*15=240

unsigned char AES_Key[32];//密钥
enum linking_mode { ECB, CBC };


//密钥扩展，产生 Nb(Nr+1) round keys
void KeyExpansion() {
    int i, j;
    unsigned char temp[4], k;

    // The first round key is the key itself.
    for (i = 0; i < Nk; i++) {
        RoundKey[i * 4] = AES_Key[i * 4];
        RoundKey[i * 4 + 1] = AES_Key[i * 4 + 1];
        RoundKey[i * 4 + 2] = AES_Key[i * 4 + 2];
        RoundKey[i * 4 + 3] = AES_Key[i * 4 + 3];
    }

    // All other round keys are found from the previous round keys.
    while (i < (Nb * (Nr + 1))) {
        for (j = 0; j < 4; j++) {
            temp[j] = RoundKey[(i - 1) * 4 + j];
        }
        if (i % Nk == 0) {
            //循环左移
            k = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = k;

            //字节代换
            temp[0] = sbox[temp[0]];
            temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]];
            temp[3] = sbox[temp[3]];

            temp[0] = temp[0] ^ Rcon[i / Nk];
        }
        else if (Nk > 6 && i % Nk == 4) {
            // Function Subword()
            temp[0] = sbox[temp[0]];
            temp[1] = sbox[temp[1]];
            temp[2] = sbox[temp[2]];
            temp[3] = sbox[temp[3]];
        }
        RoundKey[i * 4 + 0] = RoundKey[(i - Nk) * 4 + 0] ^ temp[0];
        RoundKey[i * 4 + 1] = RoundKey[(i - Nk) * 4 + 1] ^ temp[1];
        RoundKey[i * 4 + 2] = RoundKey[(i - Nk) * 4 + 2] ^ temp[2];
        RoundKey[i * 4 + 3] = RoundKey[(i - Nk) * 4 + 3] ^ temp[3];
        i++;
    }
}

// 轮密钥加
void AddRoundKey(int round) {
    int i, j;
    for (i = 0; i < Nb; i++) {
        for (j = 0; j < 4; j++) {
            state[j][i] ^= RoundKey[round * Nb * 4 + i * Nb + j];
        }
    }
}

//state字节代换
void SubBytes() {
    int i, j;
    for (i = 0; i < 4; i++) {
        for (j = 0; j < Nb; j++) {
            state[i][j] = sbox[state[i][j]];
        }
    }
}

void InvSubBytes() {
    int i, j;
    for (int i = 0; i < 4; i++) {
        for (j = 0; j < Nb; j++) {
            state[i][j] = invsbox[state[i][j]];
        }
    }
}

//state循环左移位，Offset = Row number
void ShiftRows() {
    unsigned char temp;

    // Rotate first row 1 columns to left	
    temp = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = temp;

    // Rotate second row 2 columns to left	
    temp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = temp;
    temp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = temp;

    // Rotate third row 3 columns to left
    temp = state[3][0];
    state[3][0] = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = temp;
}

void InvShiftRows() {
    unsigned char temp;
    //row 1
    temp = state[1][3];
    state[1][3] = state[1][2];
    state[1][2] = state[1][1];
    state[1][1] = state[1][0];
    state[1][0] = temp;

    //row 2
    temp = state[2][0];
    state[2][0] = state[2][2];
    state[2][2] = temp;
    temp = state[2][1];
    state[2][1] = state[2][3];
    state[2][3] = temp;

    //row 3
    temp = state[3][3];
    state[3][3] = state[3][0];
    state[3][0] = state[3][1];
    state[3][1] = state[3][2];
    state[3][2] = temp;
}

// xtime is a macro that finds the product of {02} and the argument to
// xtime modulo {1b}
#define xtime(x)   ((x<<1) ^ (((x>>7) & 1) * 0x1b))

unsigned char GFMultiplyByte(unsigned char L, unsigned char R) {
    unsigned char temp[8];
    unsigned char result = 0x00;
    temp[0] = L;
    int i;
    // temp[0] = L, temp[1] = L*x(modm(x)), temp[2] = L*x^2(mod(m(x))), temp[3] = L*x^3(mod(m(x)))...
    //先计算，再存起来，后面根据R的实际情况选用需要的
    for (i = 1; i < 8; i++) {
        if (temp[i - 1] >= 0x80) {
            temp[i] = (temp[i - 1] << 1) ^ 0x1b;
        }
        else {
            temp[i] = temp[i - 1] << 1;
        }
    }
    for (i = 0; i < 8; i++) {
        if (int((R >> i) & 0x01) == 1) {
            result ^= temp[i];
        }
    }
    return result;
}
void MixColumns() {
    unsigned char temp[4][Nb];
    for (int i = 0; i < Nb; i++) {//State的列
        for (int j = 0; j < 4; j++) {//State的行
            temp[j][i] = GFMultiplyByte(mixColumnMatrix[j][0], state[0][i]);
            for (int k = 1; k < 4; k++) {
                temp[j][i] ^= GFMultiplyByte(mixColumnMatrix[j][k], state[k][i]);
            }
        }
    }
    for (int i = 0; i < Nb; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] = temp[i][j];
        }
    }
}
void InvMixColumns() {
    unsigned char temp[4][Nb];
    for (int i = 0; i < Nb; i++) {//State的列
        for (int j = 0; j < 4; j++) {//State的行
            temp[j][i] = GFMultiplyByte(invmixColumnMatrix[j][0], state[0][i]);
            for (int k = 1; k < 4; k++) {
                temp[j][i] ^= GFMultiplyByte(invmixColumnMatrix[j][k], state[k][i]);
            }
        }
    }
    for (int i = 0; i < Nb; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] = temp[i][j];
        }
    }
}


// encrypts the PlainText.
void Encrypt() {
    int i, j, round = 0;

    for (i = 0; i < Nb; i++) {
        for (j = 0; j < 4; j++) {
            state[j][i] = in[i * 4 + j];
        }
    }

    AddRoundKey(0);

    //first Nr-1 rounds
    for (round = 1; round < Nr; round++) {
        SubBytes();
        ShiftRows();
        MixColumns();
        AddRoundKey(round);
    }

    SubBytes();
    ShiftRows();
    AddRoundKey(Nr);

    for (i = 0; i < Nb; i++) {
        for (j = 0; j < 4; j++) {
            out[i * 4 + j] = state[j][i];
        }
    }
}

// edcrypts the PlainText.
void Decrypt() {
    int i, j, round = 0;

    for (i = 0; i < Nb; i++) {
        for (j = 0; j < 4; j++) {
            state[j][i] = in[i * 4 + j];
        }
    }

    AddRoundKey(Nr);

    //first Nr-1 rounds
    for (round = Nr - 1; round > 0; round--) {
        InvShiftRows();
        InvSubBytes();
        AddRoundKey(round);
        InvMixColumns();
    }

    InvShiftRows();
    InvSubBytes();
    AddRoundKey(0);

    for (i = 0; i < Nb; i++) {
        for (j = 0; j < 4; j++) {
            out[i * 4 + j] = state[j][i];
        }
    }
}

int fillBlock(int sz, char* str, unsigned char* in) {
    int j = 0;
    while (j < Nb * 4) {
        in[j] = (unsigned char)str[sz];
        j++;
        sz++;
    }
    return sz;
}

void encrypt_aes(char* key, char* mes, char* cipher, linking_mode mode) {
    int i;
    int sel, keysize, cont = 1;
    sel = 1;
    Nk = 4;
    Nr = Nk + 6;
    memcpy_s(AES_Key, 16, key, 16);

    char str[1024];
    memcpy_s(str, 1024, mes, 1024);

    KeyExpansion();
    // sz is the cursor into the input string
    int sz = 0;
    int len_in = 1024;
    memset(in, 0, 1024);
    memset(out, 0, 1024);
    while (sz <= len_in) {
        int j = 0;
        if (mode == ECB) {
            while (j < Nb * 4) {
                in[j] = (unsigned char)str[sz];//ECB模式
                j++;
                sz++;
            }
        }
        else {
            if (sz == 0) {
                while (j < Nb * 4) {
                    in[j] = IV[j] ^ (unsigned char)str[sz];
                    j++;
                    sz++;
                }
            }
            else {
                while (j < Nb * 4) {
                    in[j] = out[j] ^ (unsigned char)str[sz];//CBC模式
                    j++;
                    sz++;
                }
            }
        }

        Encrypt();

        for (i = 0; i < Nb * 4; i++) {
            cipher[sz - Nb * 4 + i] = out[i];
        }
    }
}

void decrypt_aes(char* key, char* mes, char* cipher, linking_mode mode) {
    int i;
    int sel, keysize, cont = 1;
    sel = 2;
    Nk = 4;
    Nr = Nk + 6;
    memcpy_s(AES_Key, 16, key, 16);

    char str[1024];
    memcpy_s(str, 1024, cipher, 1024);

    KeyExpansion();

    int sz = 0;
    int len_in = 1024;
    memset(in, 0, 1024);
    memset(out, 0, 1024);
    while (sz <= len_in) {
        sz = fillBlock(sz, str, in);
        
        Decrypt();
        
        if (mode == ECB) {
            for (i = 0; i < Nb * 4; i++)
            {
                mes[sz - Nb * 4 + i] = out[i];
            }
        }
        else {
            for (i = 0; i < Nb * 4; i++)
            {
                mes[sz - Nb * 4 + i] = out[i];
                if (sz <= 16) {
                    mes[sz - Nb * 4 + i] ^= IV[i];
                }
                else {
                    mes[sz - Nb * 4 + i] ^= cipher[sz - Nb * 8 + i];
                }
            }
        }
    }
}